...
static void parse_policy()
{
    ...
    jsmn_init(&p);
    r = jsmn_parse(&p, g_policy, strlen(g_policy), t, POLICY_UPDATER_TOK_NUM);
...
{
    static char policy_request[POLICY_UPDATER_REQ_GET_LIST_SIZE] = {0, };

    snprintf(policy_request, POLICY_UPDATER_REQ_GET_LIST_SIZE, "{\"cmd\":\"get_policy\",\"policyId\":\"%.*s\"}", POLICY_UPDATER_POL_ID_BUF_LEN, policy_id);
    int response_length;
    ...
    tcp_client_send(policy_request, strlen(policy_request), response, &response_length, policy_service_address, g_policy_updater_port);
...
static unsigned int fsm_get_policy_list(void)
{
    ...
    snprintf(policy_request, POLICY_UPDATER_REQ_GET_LIST_SIZE, "{\"cmd\":\"get_policy_list\",\"policyStoreId\":\"%s\",\"deviceId\":\"%s\"}", g_policy_store_version, g_device_id);
    ...
    char policy_service_address[POLICY_UPDATER_SERV_ADDR_LEN];
    hostname_to_ip(g_policy_updater_address, policy_service_address);
    int res = tcp_client_send(policy_request, strlen(policy_request), response, &response_length, policy_service_address, g_policy_updater_port);
...
static unsigned int fsm_get_policy_list_done(void)
{
    ...
    {
        if(g_fsm_try_counter < POLICY_UPDATER_MAX_GET_TRY)
        {
...
{
    ConfigManager_get_option_string("pap", "policy_store_service_ip", g_policy_updater_address, POLICY_UPDATER_ADDRESS_SIZE);
    ConfigManager_get_option_int("pap", "policy_store_service_port", &g_policy_updater_port);
    ConfigManager_get_option_string("config", "device_id", g_device_id, POLICY_UPDATER_STR_LEN);
