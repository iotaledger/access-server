...
char g_module_name[] = "PolicyUpdater";
...
static int get_tcp_response(void *ext, char *recvBuffer)
{
    int length = 0;
...
void PolicyUpdater_get_policy(char *policy_id, char *p_policy)
{
    ...
    tcp_send(policy_request, strlen(policy_request), response, &response_length, policy_service_address, g_policy_updater_port);
...
unsigned int PolicyUpdater_get_policy_list(const char *policy_store_version, const char* device_id, char *policy_list, int *policy_list_len, int* new_policy_list_flag)
{
    ...
    snprintf(policy_request, POLICY_UPDATER_REQ_GET_LIST_SIZE, "{\"cmd\":\"get_policy_list\",\"policyStoreId\":\"%s\",\"deviceId\":\"%s\"}", policy_store_version, device_id);
    ...
    char policy_service_address[POLICY_UPDATER_SERV_ADDR_LEN];
    hostname_to_ip(g_policy_updater_address, policy_service_address);
    int res = tcp_send(policy_request, strlen(policy_request), response, &response_length, policy_service_address, g_policy_updater_port);
