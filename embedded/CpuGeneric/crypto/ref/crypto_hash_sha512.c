...
static uint64_t rotr64(const uint64_t x, const int b)
{
    return (x >> b) | (x << (64 - b));
}

static void store64_be(uint8_t dst[8], uint64_t w)
{
...
static uint64_t load64_be(const uint8_t src[8])
{
#ifdef NATIVE_BIG_ENDIAN
    uint64_t w;
...
static void SHA512_Transform(uint64_t *state, const uint8_t block[128], uint64_t W[80],
                 uint64_t S[8])
{
    int i;
