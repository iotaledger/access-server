...
static char Validator_supported_hash_fns[] = "sha-256|";
  ...
  while (next_object_index >= 0) {
    for (int j = next_object_index; j < max_idx; j++) {
      if (strncmp(&policy_data[tokens[j].start], "attribute_list", strlen("attribute_list")) == 0 &&
          tokens[j].size == 1 && tokens[j + 1].type == JSMN_ARRAY) {
        // check attribute list array recursively
        if (check_gocdoc_object(tokens, j + 2, parser_end_of_current_idx(tokens, j, max_idx), policy_data) ==
            (DG_HAS_ATTLIST | DG_HAS_OPP)) {
      ...
      } else if (strncmp(&policy_data[tokens[j].start], "operation", strlen("operation")) == 0 && tokens[j].size == 1 &&
    ...
    if (ret == DG_HAS_NONE) {
      ...
      if (tokens[next_object_index].type == JSMN_OBJECT) {
        ...
        while (next_bot_level_object_index >= 0) {
          ...
          for (int j = next_bot_level_object_index;
               j <= parser_end_of_current_idx(tokens, next_bot_level_object_index, max_idx); j++) {
            if (strncmp(&policy_data[tokens[j].start], "type", strlen("type")) == 0 &&
              ...
              if (strncasecmp(&policy_data[tokens[j + 1].start], "time", strlen("time")) == 0) {
                ...
                int operation_idx = parser_next_key_sibling_idx(tokens, att_list_idx,
                                                                parser_end_of_current_idx(tokens, att_list_idx, n) + 2);
