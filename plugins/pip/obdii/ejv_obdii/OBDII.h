 ...
 * represented by a property on the `OBDIICommands` global variable. For example, `OBDIICommands.engineRPMs` is a
 * pointer to the unique command instance for requesting the engine RPMs. The commands are also accessible via the
 * `OBDIIMode1Commands` and `OBDIIMode9Commands` global variables, which are indexed by the command's PID. For example,
 *
 ...
 *
 * The OBDII standard specifies a large number of diagnostic commands, but a particular vehicle will likely only support
 * a subset. The `OBDIIGetSupportedCommands` function can be used to query a vehicle for the set of commands it
...
/** Global variable used to obtain a reference to a mode 1 command by PID, e.g. `OBDIIMode1Commands[0x1F]` (same as
 * OBDIICommands.runtimeSinceEngineStart) */
extern OBDIICommand OBDIIMode1Commands[79];
/** Global variable used to obtain a reference to a mode 9 command by PID, e.g. `OBDIIMode9Commands[0x02]` (same as
 * OBDIICommands.VIN) */
 ...
 *
 * \returns An `OBDIIResponse` object containing the decoded diagnostic data. Make sure to call `OBDIIResponseFree` when
 * you are done with the response.
 */
