 ...
 * The returned socket uses the ISOTP transport protocol (CAN_ISOTP defined in <linux/can.h>). The ISOTP protocol is
 * implemented in a third-party kernel module that must be installed on the system. The module's source is available at
 * https://github.com/hartkopp/can-isotp-modules, which contains instructions for building and installing it.
 *
 * The ECU is identified by a (transfer ID, receive ID) tuple. The transfer ID is the ID that the ECU listens to on the
 * CAN network. The receive ID is the ID the ECU uses to respond. For vehicles that use 11-bit identifiers, ECUs have a
 * transfer ID in the range 0x7E0 to 0x7E7, and they respond with an ID in the range 0x7E8 to 0x7EF (0x08 more than
 * their transfer ID).
 *
 *     OBDIISocket s;
 ...
 * \param shared If `1`, the socket will be opened by the OBDII daemon instead of by the calling process. This allows
 * multiple processes to share the same OBDII socket, as only a single process can bind to a particular (interface,
 * transfer ID, receive ID) tuple. If `0`, the calling process will bind the socket, which may fail if another process
 * has already bound a socket for the given parameters.
 *
