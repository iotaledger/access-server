...
static int setupDaemonCommunication() {
  ...
  // Even though this is a connection-less socket, by using connect we can use send and recv calls instead of
  // sendto/recvfrom
...
OBDIIResponse OBDIIPerformQuery(OBDIISocket *socket, OBDIICommand *command) {
  ...
  // Receive the response
  int responseLength =
      command->expectedResponseLength == VARIABLE_RESPONSE_LENGTH ? MAX_ISOTP_PAYLOAD : command->expectedResponseLength;
