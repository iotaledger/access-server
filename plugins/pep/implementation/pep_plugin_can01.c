...
void can_01_pep_plugin_set_relayboard_addr(const char* addr) { strncpy(relayboard_addr, addr, sizeof(relayboard_addr)); }

static int can_01_pep_plugin_car_lock(pep_plugin_action_data_t* action, int should_log) {
  relayinterface_pulse(0);
  return 0;
}

static int can_01_pep_plugin_car_unlock(pep_plugin_action_data_t* action, int should_log) {
  relayinterface_pulse(1);
  return 0;
}

static int can_01_pep_plugin_start_engine(pep_plugin_action_data_t* action, int should_log) {
  relayinterface_pulse(2);
  return 0;
}

static int can_01_pep_plugin_open_trunk(pep_plugin_action_data_t* action, int should_log) {
...
static int can_01_pep_plugin_tcp_car_lock(pep_plugin_action_data_t* action, int should_log) {
  socket_send_byte_to_port(2222);
  return 0;
}

static int can_01_pep_plugin_tcp_car_unlock(pep_plugin_action_data_t* action, int should_log) {
  socket_send_byte_to_port(2223);
  return 0;
}

static int can_01_pep_plugin_tcp_start_engine(pep_plugin_action_data_t* action, int should_log) {
  socket_send_byte_to_port(2224);
  return 0;
}

static int can_01_pep_plugin_tcp_open_trunk(pep_plugin_action_data_t* action, int should_log) {
...
static void init_ds_interface(dataset_state_t* vdstate) {
  ...
  vdstate->options = &vehicledataset_can_01_pep_plugin_options[0];
...
void can_01_pep_plugin_initializer(pep_plugin_t* action_set, void* options) {
  ...
  g_action_set->actions[0] = can_01_pep_plugin_car_unlock;
  g_action_set->actions[1] = can_01_pep_plugin_car_lock;
  g_action_set->actions[2] = can_01_pep_plugin_open_trunk;
  g_action_set->actions[3] = can_01_pep_plugin_start_engine;
...
void can_01_pep_plugin_initializer_tcp(pep_plugin_t* action_set, void* options) {
  ...
  g_action_set->actions[0] = can_01_pep_plugin_tcp_car_unlock;
  g_action_set->actions[1] = can_01_pep_plugin_tcp_car_lock;
  g_action_set->actions[2] = can_01_pep_plugin_tcp_open_trunk;
  g_action_set->actions[3] = can_01_pep_plugin_tcp_start_engine;
  strncpy(g_action_set->action_names[0], "open_door", RES_ACTION_NAME_SIZE);
  strncpy(g_action_set->action_names[1], "close_door", RES_ACTION_NAME_SIZE);
  strncpy(g_action_set->action_names[2], "open_trunk", RES_ACTION_NAME_SIZE);
  strncpy(g_action_set->action_names[3], "start_engine", RES_ACTION_NAME_SIZE);
...
void can_01_pep_plugin_terminizer() { g_action_set = NULL; }
