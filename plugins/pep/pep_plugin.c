 ...
 * GLOBAL VARIABLES
 ****************************************************************************/
//@TODO: This can be removed by adding state and timer to pep_plugin_t, which will be passed as functions argument
static char action_s[] = "<Action performed>";
static int timerId = -1;
static dataset_state_t *g_dstate = NULL;
static pep_plugin_t pep_plugin_action_set = {0};
...
static int start_data_sharing(const char *action, unsigned long end_time) {
  ...
  pep_plugin_action_set.init_ds_interface_cb(g_dstate);
  ...
  pep_plugin_action_set.start_ds_interface_cb();
...
static int stop_data_sharing() {
  timer_stop(timerId);
  pep_plugin_action_set.stop_ds_interface_cb();

  return 0;
}

static int action_resolve(pep_plugin_action_data_t *action, int should_log) {
  ...
  if (0 == memcmp(action->value, "start_ds_", strlen("start_ds_") - 1)) {
    ...
    for (int i = 0; i < pep_plugin_action_set.count; i++) {
      if (memcmp(action->value, pep_plugin_action_set.action_names[i], strlen(pep_plugin_action_set.action_names[i])) ==
          0) {
        timemanager_get_time_string(buf, RES_BUFF_LEN);
        dlog_printf("%s %s\t%s\n", buf, action, action_s);
        retval = pep_plugin_action_set.actions[i](action, should_log);
        break;
      }
    }
  }
...
static bool pep_plugin_callback(char *obligation, void *action) {
  bool should_log = FALSE;
  pep_plugin_action_data_t *action_r;

  if (action == NULL) {
    dlog_printf("\nERROR[%s]: Bad input parameter.\n", __FUNCTION__);
    return FALSE;
  } else {
    action_r = (pep_plugin_action_data_t *)action;
  }
...
pep_plugin_fn pep_plugin_init(pep_plugin_initializer_t initializer, dataset_state_t *dstate) {
  initializer(&pep_plugin_action_set);
  g_dstate = dstate;

  return pep_plugin_callback;
}

void pep_plugin_term(pep_plugin_terminizer_t terminizer) {
  terminizer(&pep_plugin_action_set);
  g_dstate = NULL;
