...
const decode = (payload, side_key, root) => {
    const mam = Mam.decode(payload, side_key, root);
    return mam;
};
...
const txHashesToMessages = async (hashes) => {
    ...
    const objs = await getTransactionObjects(hashes);
    return objs
...
const hash = (data, rounds) => {
    return converter.trytes(Encryption.hash(rounds || 81, converter.trits(data.slice())).slice());
};
const keyGen = length => {
